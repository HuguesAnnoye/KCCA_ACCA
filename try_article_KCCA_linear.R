...
Y11 <- rnorm(1000,0,1)
Y12 <- rnorm(1000,4,10) - Y11
Y13 <- 3*X12 + 4 *Y11 + rnorm(1000,0,1)
Y14 <- 5*X13 + 2*X14 + Y11  + rnorm(1000,0,1)
...
Y21 <- rnorm(1000,0,1)
Y22 <- rnorm(1000,4,10) - Y21
Y23 <- 3*X22 + 4 *Y21 + rnorm(1000,0,1)
Y24 <- 5*X23 + 2*X24 + Y21  + rnorm(1000,0,1)
...
print(temps)
#Save Results
save(list=ls(), file="~/resultat/25_novembre_2024/KCCA/Essais_KCCA_linear.RDATA")
