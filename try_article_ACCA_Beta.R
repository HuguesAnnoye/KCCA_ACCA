...
set.seed(46)
a <- 0.2
Y11 <- a*rbeta(1000,2,2)
X11 <- rnorm(1000,0,1)
X12 <- rnorm(1000,1,0) + Y11
...
Y <- cbind(Y11,Y13,Y14,Y15,Y16,Y17)
colnames(Y) <- paste0("NCV_",1:6)

#Create test set
Y21 <- a*rbeta(1000,2,2)
X21 <- rnorm(1000,0,1)
X22 <- rnorm(1000,1,0) + Y21
X24 <- rnorm(1000,0,10)
sig <- rbind(c(1,0.5,0.5),c(0.5,1,0.5),c(0.5,0.5,1))
...
Y2 <- cbind(Y21,Y23,Y24,Y25,Y26,Y27)
colnames(Y2) <- paste0("NCV_",1:6)
...
#Save Results
save(list=ls(), file="~/resultat/25_novembre_2024/ACCA/Essais_ACCA_Beta.RDATA")
